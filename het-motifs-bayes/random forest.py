...
# 交叉验证并计算指标
i=0
f_data = open('TwiBot22_10折交叉验证_userID.csv', 'w')
f_data.write('折数'+','+'训练集用户'+','+'测试集用户'+'\n')
for train_index, val_index in skf.split(Xtrain, Ytrain):
    ...
    # 打印出每折的UserID
    # train_userID = df.iloc[train_index,0].tolist()
    # val_userID = df.iloc[val_index,0].tolist()
    # f_data.write(str(i+1)+','+'、'.join(train_userID)+','+'、'.join(val_userID)+'\n')
    ...
    for ii in n_est:
        for jj in m_depth:
            ...
            clf_proba = clf_rf.predict_proba(X_val_fold)

            y_pred = clf_rf.predict(X_val_fold)
